...
function assertType(type) {
  ...
  if (![NODE_TYPE.SYSTEM, NODE_TYPE.MODULE, NODE_TYPE.FUNCTION].includes(t)) {
    throw Object.assign(new Error(`Invalid type: ${type} (expect 0=系统 1=模块 2=功能)`), { code: 'VALIDATION' });
  }
  return t;
}
...
export async function create(data) {
  ...
  if (type === NODE_TYPE.SYSTEM && parentId != null) {
    throw Object.assign(new Error('type=0（系统）的 parentId 必须为空'), { code: 'VALIDATION' });
  }
  ...
  if (parentId != null) {
    ...
    if (type === NODE_TYPE.MODULE && parent.type !== NODE_TYPE.SYSTEM) {
      throw Object.assign(new Error('模块的父节点必须是系统（type=0）'), { code: 'VALIDATION' });
    }
    if (type === NODE_TYPE.FUNCTION && parent.type !== NODE_TYPE.MODULE) {
      throw Object.assign(new Error('功能的父节点必须是模块（type=1）'), { code: 'VALIDATION' });
    }
  }
...
export async function update(id, data) {
  ...
  for (const key of allowed) {
    if (data && Object.prototype.hasOwnProperty.call(data, key) && data[key] !== undefined) {
      patch[key] = key === 'name' ? String(data[key]).trim() : data[key];
    }
  }
