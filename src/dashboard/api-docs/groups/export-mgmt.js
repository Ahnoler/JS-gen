...
export const GROUP_EXPORT = [
  ...
  },
  {
    id: 'batch-export',
    name: '批量导出管理',
    description: 'Partner transaction 导出（单轨 / 批量）。导入体为 { transcationEventTypeList: [ { …, transcationProperties: [...] } ] }；拼写（transcation*、mothed）为对接约定。成功导出后 trajectory.is_export=1；phase/step 变更置 0。',
    endpoints: [
      {
        method: 'GET', path: '/api/v2/export/transaction/schema',
        summary: 'Partner transaction 字段契约',
        desc: '返回导入体字段说明（外层 transcationEventTypeList；轨内 transcationProperties 等）、eventTypeName 中文映射与 action→type 映射。拼写（transcation*、mothed）为对接约定，勿改。',
        respExample: J({
          schemaVersion: 1,
          fields: [
            { key: 'transcationEventTypeList', zh: '交易列表（单轨也包一层数组）' },
            { key: 'transcId', zh: '录制/交易 id（轨内可选）' },
      ...
      },
      {
        ...
        desc: '全量导出 trajectory_step 为 partner envelope；成功后将 trajectory.is_export 置 1。download / raw / forImport=1 时响应体仅为 envelope（可直接 POST 到对方 importDemand）。',
        params: [
          { name: 'id', type: 'number', required: true, in: 'path', desc: '轨迹 id', example: '53' },
          { name: 'systemId', type: 'string', required: true, in: 'query', desc: '系统树 id', example: '98' },
          { name: 'projectId', type: 'string', required: true, in: 'query', desc: '项目 id', example: '31' },
          { name: 'raw', type: 'boolean', in: 'query', desc: 'true 时仅返回 envelope JSON（联调推荐）', example: 'true' },
          { name: 'forImport', type: 'boolean', in: 'query', desc: '同 raw，语义化为导入联调', example: 'true' },
          { name: 'download', type: 'boolean', in: 'query', desc: 'true 时仅返回 envelope，并带 Content-Disposition 附件头', example: 'false' },
        ],
        ...
        notes: [
          ...
          'raw/forImport 响应即 importDemand body：外层必含 transcationEventTypeList',
          'propertiesName 无分隔符（如 点击客户管理）；同轨内不可重复，重复则追加 2、3…',
        ],
      },
      {
        method: 'POST', path: '/api/v2/export/trajectories/{id}/transaction',
        summary: '导出轨迹（body 传 systemId/projectId）',
        desc: '与 GET 相同；systemId/projectId/raw/forImport/download 可写在 body 或 query（body 优先）。联调对方 importDemand 时请 raw/forImport=true，响应即导入 body。',
        params: [
          { name: 'id', type: 'number', required: true, in: 'path', desc: '轨迹 id' },
          { name: 'systemId', type: 'string', required: true, in: 'body', desc: '系统树 id' },
          { name: 'projectId', type: 'string', required: true, in: 'body', desc: '项目 id' },
          { name: 'raw', type: 'boolean', in: 'body', desc: 'true 时仅返回 envelope' },
          { name: 'forImport', type: 'boolean', in: 'body', desc: '同 raw' },
          { name: 'download', type: 'boolean', in: 'body', desc: 'true 时仅返回 envelope + 附件头' },
        ],
        reqExample: J({ systemId: '98', projectId: '31', raw: true }),
      },
      {
        method: 'POST', path: '/api/v2/export/transactions',
        summary: '批量导出 partner transaction',
        desc: '逐条独立 ok/fail；成功项 markExported。raw/forImport=true 时合并为一条 importDemand body（transcationEventTypeList 含多轨）。',
        params: [
          { name: 'trajectoryIds', type: 'number[]', required: true, in: 'body', desc: '轨迹 id 列表（亦支持逗号分隔字符串，parseIdList）', example: '[53,54]' },
          { name: 'systemId', type: 'string', required: true, in: 'body', desc: '系统树 id' },
          { name: 'projectId', type: 'string', required: true, in: 'body', desc: '项目 id' },
          { name: 'raw', type: 'boolean', in: 'body', desc: 'true 时返回合并后的 importDemand body' },
          { name: 'forImport', type: 'boolean', in: 'body', desc: '同 raw' },
        ],
        reqExample: J({ trajectoryIds: [53, 54], systemId: '98', projectId: '31', raw: true }),
        respExample: J({
          schemaVersion: 1,
          systemId: '98',
          projectId: '31',
