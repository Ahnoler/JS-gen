    ...
    endpoints: [
      ...
      },
      {
        ...
        notes: [
          ...
          'transcationProperties 统一 schema，截图条目与控件步骤条目同构；用 type 字段区分条目种类（§8 层级类型）：page=页面截图、popup=弹窗截图、tab=tab页签、section=折叠面板、wizard=步骤向导面板、card=卡片、object=业务对象',
          '中间节点：type=section/tab/wizard/card（按 region_id 段 role 映射），用 propertiesID/propertiesPID 表达分区父子层级（同页同名控件可区分）；无截图/坐标/action 字段（screenshot=[]、rect=""、elementType/eventTypeValue 空）；main/shell-header/shell-aside/other 等结构性 role 跳过不建节点',
          ...
          '控件步骤条目：eventTypeValue=click/input/...、elementType=xpath、mothed=By.XPATH、type=object、screenshot=[]空数组、rect=坐标 JSON 字符串或""；弹窗内控件 rect 相对弹窗截图',
