...
export async function analyzeRequirementToPhases({
  ...
  const prompt = [
    ...
    '',
    '【阶段拆分规则 — 必须遵守】',
    '1. 同类型操作合并原则：连续的相同类型操作应合并为一个阶段。例如多个"点击"操作合并为一个阶段，多个"填写表单"操作合并为一个阶段，多个"选择下拉框"操作合并为一个阶段。判断类型依据：操作动作（点击/填写/选择/勾选/滚动等）+ 操作对象类型（菜单/按钮/输入框/下拉框/复选框等）。',
    '2. 识别编号格式如「1、」「1.」「1)」「（1）」等作为步骤边界参考，但不强制每条编号一个阶段——同类型操作即使跨编号也应合并。',
    ...
    '9. 合并后阶段数量控制：一般场景不超过5个阶段，复杂场景不超过8个阶段。如果用户步骤很多但类型相似，大胆合并。',
    '',
    '【预期结果规则 — 必须遵守】',
