...
export async function analyzeRequirementToPhases({
  ...
  const prompt = [
    ...
    '3. 必须单独成阶段的情况：页面跳转/导航类操作（如"点击菜单进入XX页面"）应独立为阶段，因为它是后续操作的前置条件；涉及不同页面/弹窗的操作应分不同阶段。',
    '3.1 状态边界原则（硬性）：阶段边界必须落在「稳定、可判定的页面状态」上，且相邻阶段不得共享同一瞬态弹窗的触发与确认。具体：(a) 若某阶段的预期结果是「打开某确认弹窗/提示弹窗」，则该阶段必须完整包含把弹窗打开的全部动作（如「在列表中选中该行」+「点击【删除】按钮」），并在此终止；弹窗内按钮的点击（如「点击【确定】」）只归下一阶段。(b) 禁止把同一动作单元（如"选中行→点删除"）拆到两个阶段，也禁止让下一阶段承担上一阶段未完成的动作——下一阶段的每个动作都必须在上阶段预期结果已成立的前提下才可能执行。(c) 若上一阶段的预期结果可能不是确定可达的（存在中间校验、异步刷新等），则把「触发 → 确认」合并为一个阶段，不要为「点确定」单独切一个阶段。',
    ...
    '6.1 描述精简原则：阶段描述只保留操作本身和必要的操作对象/目标值，删除操作无关的详细说明。具体规则：(a) 保留：操作动词+操作对象+具体值（如"点击【更多】按钮"、"填写客户名称为"测试客户""）；(b) 删除：操作后的效果列举、字段罗列、功能说明等详细描述（如"展开审批状态、评级发生类型、申请日期等查询条件字段"这种列举式说明应删除）；(c) 保留：指导如何操作的限定词（如"在搜索框中输入"中的"在搜索框中"需要保留，因为它指导操作位置）。',
    ...
    '8. 若某步描述「定位/选中」侧栏树节点或列表行：在同一 phase 字符串内补写「先搜索/查询再点击」纪律，且必须原样保留原文中的目标名称与取值（例如「先搜索「KB测一级-…」再点击该节点」）；禁止把具体名抹成笼统的「关键字/对应节点」。',
