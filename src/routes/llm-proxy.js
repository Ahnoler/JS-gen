...
async function handleStandaloneCompletion(req, res) {
  if (!LLM_BASE_URL || !LLM_API_KEY) {
    return res.status(500).json({ error: { message: 'LLM_BASE_URL and LLM_API_KEY env vars are required in standalone mode', type: 'server_error' } });
  }
  const { model: modelId, messages, temperature, tools, tool_choice } = req.body || {};
...
export default function (app) {
  ...
  app.get('/v1/models', async (req, res) => {
    if (STANDALONE_LLM) {
      if (!LLM_BASE_URL || !LLM_API_KEY) {
        return res.status(500).json({ error: { message: 'LLM_BASE_URL and LLM_API_KEY env vars are required in standalone mode' } });
      }
      try {
  ...
  app.post('/v1/chat/completions', async (req, res) => {
    ...
    if (!state.client && !STANDALONE_LLM) return res.status(503).json({ error: { message: 'opencode server not ready', type: 'server_error' } });
    if (stream) return res.status(400).json({ error: { message: 'streaming not supported', type: 'invalid_request_error' } });

    if (STANDALONE_LLM) {
      return handleStandaloneCompletion(req, res);
    }
