...
export default function (app) {
  ...
  app.get('/api/v2/screenshots/pending', asyncHandler(async (req, res) => {
    const list = await screenshotService.listPendingScreenshots();
    res.json(list);
  }));

  // One-click bulk upload of all local-pending screenshots to MinIO.
  app.post('/api/v2/screenshots/pending/upload', asyncHandler(async (req, res) => {
  ...
  app.post('/api/v2/screenshots/:id/upload', asyncHandler(async (req, res) => {
  ...
  app.get('/api/v2/screenshots/:id/image', asyncHandler(async (req, res) => {
  ...
  app.delete('/api/v2/screenshots/:id', asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isFinite(id) || id <= 0) {
      return res.status(400).json({ error: 'Numeric screenshot id required' });
    }
  ...
  app.get('/api/v2/trajectories/:trajectoryId/screenshots', asyncHandler(async (req, res) => {
