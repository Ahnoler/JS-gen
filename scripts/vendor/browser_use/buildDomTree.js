/*
 * Vendored from browser_use 0.1.48
 * (python/Lib/site-packages/browser_use/dom/buildDomTree.js, upstream unmodified except where noted).
 *
 * 本仓补丁（#910④）：isTopElement 主文档分支在祖先链 miss 后增加 Element-UI
 * 浮层豁免——topEl.closest('.el-popper') 命中且该 popper 不含 el-tooltip__popper
 * 类时仍视为 top。生产事故：el-dialog footer「确 定」被「请选择」触发的
 * el-select tree-popover 展开遮挡，上游判非 top → 不分配 index → agent 元素表
 * 没有确定钮。el-dialog / .el-overlay 不属 .el-popper 家族，模态真遮挡行为不变；
 * tooltip 豁免维持关闭。其余分支（viewportExpansion === -1 / iframe / shadow DOM）
 * 逐字保留上游原样。
 *
 * 与上游同步须知：升级 browser_use 版本时，以新版 buildDomTree.js 为基础重放本
 * 补丁，并同步更新 scripts/characterization/characterize-domtree-occlusion.mjs
 * 的 needle 断言；本副本由 patch_dom_tree_js（scripts/agent_utils.py）在运行时
 * 覆写 browser_use.dom.service.DomService.js_code 生效。
 */
(
  args = {
  ...
  function isTopElement(element) {
    ...
    try {
      ...
      // 本仓补丁（#910④）：Element-UI 浮层瞬态遮挡豁免。祖先链 miss 后，若命中
      // 元素落在 .el-popper 浮层内（tooltip 浮层除外），候选元素仍视为 top
      // ——例如 el-select 展开的 tree-popover 盖住 el-dialog footer「确 定」时，
      // 上游会误判候选非 top 而不分配 index。el-dialog / .el-overlay 不属
      // .el-popper 家族，模态真遮挡行为不变。
      try {
        if (
          topEl &&
          typeof topEl.closest === 'function' &&
          topEl.closest('.el-popper') &&
          !topEl.closest('.el-tooltip__popper')
        ) {
          return true;
        }
      } catch (e) {
