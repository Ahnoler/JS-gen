...
2026-09-21 retarget: the confirm/notification logic moved from _misc.py to
ClickEngine (d9ca7990, click_record/replay unify). Split assertions:
  A  get_page_state notifications collector  -> scripts/controller/actions/_misc.py
  B/C/regression watcher + tokens + classify -> scripts/controller/actions/click_action_engine.py
"""
...
def main() -> int:
    ...
    engine = (ROOT / 'scripts/controller/actions/click_action_engine.py').read_text(encoding='utf-8')
    ...
    watcher_pos = engine.find('JS_WATCH_SAVE_NOTIFICATIONS')
    err_pos = engine.find('err-notification:')
    toast_pos = engine.find("record_success_token(self.business_data_store, 'toast_ok'")
    success_pos = engine.find("record_success_token(self.business_data_store, 'confirm_click'")
    assert_true(
    ...
    assert_true('状态更新成功' in engine, 'enable/status-update success text recognized')
    assert_true("return { errors, successes }" in engine, 'confirm note returns errors+successes')
    assert_true('confirm success notification → toast_ok' in engine, 'success path logs toast_ok')
    ...
    assert_true(
        engine.count("if compact.startswith(('确认', '确定')):") == 1,
        'bare compact confirm check may only appear once (inside label guard)',
    )
    assert_true(
        "if btn_label and re.sub(r'\\s+', '', btn_label).startswith(('确认', '确定')):" in engine,
