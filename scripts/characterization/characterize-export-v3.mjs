...
function testBuildV3Properties() {
  ...
  check(first.type === 'ele', 'type=object');
...
function testPayloadStructure() {
  ...
  check(props[2].type === 'ele' && props[2].propertiesPID === props[1].propertiesID, '控件条目 propertiesPID 指向 tab 条目 propertiesID');
  ...
  // rect_norm 优先：elementJson 带 rect_norm 的控件直出归一化值，stats.normalizedRects >= 1
  {
    ...
    const normCtrl = normProps.find((p) => p.type === 'ele');
