...
### Phase E — 逐模块逐叶湿测（A→B→C 团队流水线，2026-09-05 定型 / 2026-09-06 v5 增补）

切片验收后的独立任务单元，**一次只推进一个模块**（用户拍板），三角色分工：

- **A 预备代理（文本，可与 B 前段并行）**：从 chapters 的清单行建 `data/kb/req/<key>/wet-test.md` 判定表（SKILL 湿测节模板与词表，**清单行三形态**）；存量模块章末缺标准清单行的先回补（SKILL「存量回补条款」）。
...
**Step 0 入口可达性预检**（v5，移动端/H5/外部系统类模块）：建表前先探测 SUT 入口（菜单树/路由/微前端注册表五层法，见 digital-mobile 探测报告）——不可达则整模块 not-found + 环境补测条件，不建空表逐行空判。

**大模块两棒接力**（v5，>80 叶）：拆 B1/B2 两棒，B1 在运行记录注明接力点，B2 重派时读已填行了解口径续跑——配合增量写回，回传故障零浪费（asset 两模块 85+98 / 77+104 实证）。

**Lead 验收线（每模块 B 收口必做，不过关打回重测）**：

0. **机械闸门**：`node scripts/kb/wet-test-check.mjs <moduleKey>` 无 FAIL（叶集 diff/判定统计/blocked-drift 证据校验/drift 回填覆盖；支持三形态叶）；
...
节奏：会话倒计时实为请求续期，持续操作窗口更长——**一个模块一个登录窗口**仍成立；推进顺序按业务链（30/30 已收口：授信域 4 → 客户域 4 → 用信域 2 → 放还款 2 → 贷后 3 → 催收 → 产品 → 档案 → 智控 → 门户 → 资产保全 2 → 数字化 2 → 额度 2 → 会议 → 系统）。
...
### B 湿测代理提示词模板（Phase E，蒸馏卡——派发时逐项保留硬约束；v5 增补接力/同构/特殊形态）
...
必读：SKILL.md「湿测（视图3）」节（判定词表/写操作黑名单/执行规则/实测坑清单）；
data/kb/req/<KEY>/wet-test.md（判定表，逐行回填）；chapters/ 各章（文档口径）。

SUT：http://test.creditv5p2.tansun.com.cn/。会话倒计时实为请求续期；若跳 #/login：
用户名 701994、密码 1，验证码/手机验证码留空，点【登 录】；中途过期重登继续。

派发前 Lead 已确认：
- 指定账号可登录性（meeting-mgmt 教训：角色名≠账号名，先预验）；
- 模块形态（正常 ZJJK / 无编号占行 —（页面名） / 接口分册接口号叶——判定口径见 SKILL）；
- 入口可达性（Step 0 预检结论；不可达模块整模块 not-found 不建空表）。

铁律：
- 写操作黑名单：确定/提交/保存/暂存/删除/作废/撤销/同意/复制/失效等落库动作一律不点；
  向导/弹窗/抽屉核对结构即止，截图后关闭；**无确认框的删除按钮禁止触发**（先实证确认框存在）。
- 只读入口优先【查看/查看详情】；无可达记录判 blocked+补测条件（注明
  无数据/黑名单/校验拦截/静默拦截 成因）。
- 每行判定含日期（可判定词同格）；blocked 留异常原文；drift 必标类别。
- 截图 tmp/kb-wet-test/<KEY>/<编号>-<ZJJK>-<slug>.png。
- **链组增量写回**：每完成一组立即写回 wet-test.md，禁止攒最后。
- **同构页批量核验**（>40 叶模块）：首叶全量取证，后续同构叶证据列写「同叶N + 差异点」。
