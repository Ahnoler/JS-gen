...
- `taskDraft`：一项业务能力 + 一次落库闭环。准备步骤（定位/搜索/选中 `dataDependsOn`、打开该能力入口、填该能力字段）从属于这一项能力。
...
<examples>
<good>
根能力、一次落库、`produces` 非空、无上游（对齐湿测 #675 形态，规则通用）。
...
<good>
维护已有对象：定位 → 填**该项能力**字段 → 一次落库。准备步骤从属于这一项能力。
...
同页把维护与另一次可独立验收的列表侧能力（例如核对排序结果）写进同一 `taskDraft`。同页、同对象不是合并理由。应各成 atom：维护笔只含定位→填本能力字段→一次保存；另一项可独立验证的能力单独成 atom。
