...
1. **一个写操作步骤 → 一个 atom**（写操作含：保存、提交、启用、禁用、克隆、修改、删除，以及会落库/改状态的新增/创建/录入等）。
2. **禁止**把同一条主链上的多个写操作合并进一个 atom；**禁止**把整条 `主链 A` 打成一笔交易。
3. 纯导航/加载步骤（进入、加载、刷树、打开页面）以及**仅打开新增/向导抽屉、无保存/提交**的入口步骤（如「点【新增】打开向导抽屉」）**不单独成 atom**，必须并入下一个真正写操作的 `taskDraft` 前序；若主链仅有导航/入口步骤，则可单独成 atom。
4. `stepIndexes` 列出本 atom 覆盖的步骤序号（1-based）；写 atom 通常只含一个写步骤序号，导航/入口并入时可在 `taskDraft` 体现但不额外增加写步骤序号。
