...
- `pending_labels` / `readonly_labels` 仅为短名单；完整定位以 `pending_items` / `readonly_items` 为准（`readonly_*` 为已禁用字段，常有值，作业务参考，**不要** fill/select）。
- **壳层导航**（侧栏/顶栏菜单）不要依赖本清单；建任务列表用 **`scan_form_fields()`**，批量填用 **`run_form_assistant()`**（合约 `allow_form_assistant=true` 时）。
- **`run_form_assistant(region='')` — 批量扫描并自动填写当前容器内可编辑字段。** 可选 `region=` 收窄到某一折叠/Tab/卡片区域（标题见返回的 `sections[]` / `region_label`）。仅在【阶段意图合约】`allow_form_assistant=true` 时调用（典型：表单填写、表单修改—全部字段）。导航/查询阶段禁止调用；单字段 `fill_*` / `select_*` 不会触发助手。返回 `ok |` 后接 JSON：`status`（如 `auto-fill-complete`）、可选 `needs_agent[]`（`{label, reason}` — 助手跳过、须你亲自填）、`sections[]`（各区块含 `section_id`/`section_title`/`region_label`、`fields_total`、`fields_editable_pending`、`fields_sample`、`buttons`）、可选 `ambiguous_buttons[]`（跨区域同名按钮）。助手结果是草稿（见下方「你需要的纪律」段：先处理 `needs_agent` 并终检再保存）。`get_pending_tasks()` 仍可含 `NEXT_ACTION: click_save(..., region='…')`（有唯一保存块或你已传 scope 时）。
...
通过显式动作 `run_form_assistant()` 批量扫描并填写表单字段。助手填的是**草稿**：可能漏字段或填错关联约束——**不可默认正确**，保存前必须终检。
...
- **导航/查询阶段：** 禁止调用 `run_form_assistant`；不要为了后续阶段提前打开「修改/维护/新增」等入口（见合约 `out_of_scope`；【阶段目录】中后续阶段一律 out of scope，只执行当前阶段）。

**助手的行为（`run_form_assistant` 成功后）：**
- **KB-I5 引擎能力（2026-09-02）：** ①分步向导抽屉（步骤条 / 下一步+上一步 / 流程提交·意见 并存）**不会被误判为查询工具栏**——含查询区的向导步骤可直接扫描与调用助手，勿因「有查询按钮」而跳过；②选择器弹窗（引入/选择客户等）按具体场景实际发生的有效动作逐步执行和记录，搜索字段、查询、选行、确认等只是可能出现的动作，不要求固定四步；禁止为了凑步骤而补操作。日期、下拉、树形搜索以及任务明确要求的其他专用组合控件才允许封装。确认后引擎做回填补验，失败时动作返回 `err-refill-not-verified`——此时应重试该次选择（重新查询并单选确认），不要当作已填；③`login` 会自动识别「已登录复用」（同账号直接 ok）与「孤儿/异账号会话」（清缓存重登），重复登录同一账号不会失败。
...
- 若 `ambiguous_buttons` 含「保存」等多处同名按钮，须 `click_save('保存', region='…')` 指定区域标题。不要再用 `select_option(..., "first")` 批量重选（盲目找保存按钮见『任务列表规则』严禁条）。
- **每步最多 1 个 select_option**：下拉打开/关闭会改变页面状态，禁止在同一 action 列表中连续选择多个字段。`xpath-not-found` / `no-items` 后重新扫描（见「表单字段规则」§4）复制精确 `xpath_smart`；`no-items` 后禁止用 `click_element_by_index` 点 el-option，重新扫描后最多再调用一次 `select_option`。
- **性别等 radio 字段用 `click_radio`，不要用 `select_option`。**
- **只填写/修改任务明确提到的字段**（表单填写时其余交给助手；部分修改时未提及的保留）；若任务/【业务数据】要求的值与助手填的不一致，**覆盖为场景要求值**，否则保留。
- **不要**在不需要填表的页面调用 `scan_form_fields` 或 `run_form_assistant`。

# 🚨 表单字段规则（CRITICAL — 不可忽略）
1. **`input_text` 不可用** — el-form-item 内文本/密码/多行输入框一律 `fill_form_field(label_text, value, xpath_smart='')`，通过相对 `xpath_smart` 定位。
2. **写路径 xpath 优先：** `fill_form_field` / `select_option` / `click_radio` / 日期填写均支持可选 `xpath_smart`；扫描或 pending 项有 `xpath_smart` 时**务必带上**，勿只靠 label。`label_text` 仅语义，须与扫描/`get_pending_tasks` 中的 `label` **完全一致**（含 placeholder 作 displayName 时照抄）。
3. **若返回 `ambiguous-label`：** 同 label 对应多个 xpath — 从扫描/pending 复制**精确** `xpath_smart` 重试，勿换猜别的 label。
...
**表单修改：** 写入范围以合约 `refill` 为准——`all_editable`=对每个可编辑字段执行写动作（**可同值重填**，为录制可操作元素）；`touched`/`none`=只写任务点名字段、其余保留原值。之后 `click_save` → 成功判据见『校验与提交规则』 → `done`。

**🚨 严禁：** ① 在 `already-matched` / `pending:[]` / `NEXT_ACTION: click_save` 之后继续逐个重选民族/学历/婚姻状况等（浪费步数且改坏级联表单）；② 用 `scroll_down`/`scroll_up` + `click_element_by_index` 盲目寻找「保存」——必须用 `click_save()`。

**🚨 核心纪律（AI 录制）：** 对每个可编辑字段须有写动作记录（fill/select/radio）；允许填入与回显相同值。不要仅 `check_field_value` 后提交。引入/选人弹窗点「确认」即成功，不要求操作成功 toast。**⚠️ 切换 Tab 前必须点「暂存」（el-tabs 切换会丢失未保存数据）。**
