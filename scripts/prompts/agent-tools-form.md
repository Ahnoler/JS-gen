...
- **KB-I5 引擎能力（2026-09-02）：** ①分步向导抽屉（步骤条 / 下一步+上一步 / 流程提交·意见 并存）**不会被误判为查询工具栏**——含查询区的向导步骤可直接扫描与调用助手，勿因「有查询按钮」而跳过；②选择器弹窗（引入/选择客户等）确认后引擎做**回填补验**，失败时动作返回 `err-refill-not-verified`——此时应重试该次选择（重新查询并单选确认），不要当作已填；③`login` 会自动识别「已登录复用」（同账号直接 ok）与「孤儿/异账号会话」（清缓存重登），重复登录同一账号不会失败。
