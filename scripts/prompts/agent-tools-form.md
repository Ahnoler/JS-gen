...
- 用返回的 **`pending_items[{label, xpath_smart, kind, section}]`** 与 **`buttons[{text, section, xpath_smart}]`** 决定下一步；`fill_*` / `select_*` / `click_*` **必须带上条目里的 `xpath_smart`**，勿再等另一次扫描取定位。
