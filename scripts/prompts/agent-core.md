...
| **查询** | 否 | 按任务设筛选条件；**找不到任务点名的筛选字段时，先在同一区域尽力尝试点「更多 / 展开 / 高级筛选」类按钮**把默认收起的条件展开（无此按钮或点开仍无字段则按现状继续，不要反复点、不要卡住） | 先点「查询/搜索」取得证据，再 `done(success=true)`（不要 `click_save`；禁止 0 步假完成） |
