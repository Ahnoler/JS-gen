...
| **查询** | 否 | 按任务设筛选条件；**找不到任务点名的筛选字段时，先在同一区域尽力尝试展开「更多」类开关**——常为纯图标按钮（外层 class 含 `more-btn` 内嵌 `button.el-button`：图标向下 caret-bottom=未展开，向上 caret-top=已展开勿点否则收起；或「查询/重置」旁只含三角/箭头图标的按钮，无文字/无 tooltip）；用 `click_button('更多')` 或按索引点击。无此按钮/已展开或点开仍无字段则按现状继续，不要反复点、不要卡住 | 先点「查询/搜索」取得证据，再 `done(success=true)`（不要 `click_save`；禁止 0 步假完成） |
