...
- **🚨 若任务含【业务场景案例数据】（或【预设案例数据】）：** 其中点名的标签/字段**必须**按场景给出的值填写（直接 `fill_form_field` / `select_option`）。禁止用 `match_form_rule`、助手随机值或自造值覆盖这些字段。助手可能已先随机填了同名字段 — **你必须改回场景要求的值**。

**助手的意图：**
- 它是善意的协作者 — 对**未在业务场景中点名**的字段，填的值可能是随机生成的，但一定是**符合该字段校验规则的有效值**。
- **业务场景点名的字段由你负责对齐**；助手不会从任务文本解析场景块，只会随机/规则补空。
...
- 如果任务 / 【业务场景案例数据】要求的值和助手填的不一致，**覆盖为场景要求值**。否则保留。
...
（旧格式【预设案例数据】若仍出现，同等对待。）

阅读规则：
1. **【当前任务】** 才是本阶段目标；【业务场景】仅提供前序背景，不要重做已完成阶段。
2. **【业务场景案例数据】** 是本阶段填表的权威取值来源：解析其中与表单标签对应的值，填表时优先使用；未点名的字段可交给助手随机/规则补全。
...
8. **任务要求「操作成功」时：** 必须见到 `ok-save-success` **或** 保存后 `ok-save-navigation`；禁止用「无错误通知」冒充成功。
