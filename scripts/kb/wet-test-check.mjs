  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (noCodes) {
      ...
      for (const g of noCodes) {
        ...
        // 排除契约文档自身的格式说明字面量
        if (name !== '页面名' && name !== '无编号') leaves.push('NOZJJK:' + name.split('·')[0]);
      }
    }
  }
...
function parseWetTest(filePath) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const verdictMatch = line.match(/\|\s*(match|drift|blocked|not-found|pending)\b[^|]*\|/);
  ...
  for (const r of rows) {
    ...
    if (noCodeMode) {
      for (const g of r.line.match(/—（[^）]*）/g) ?? []) {
        const name = g.slice(2, -1);
        if (name !== '页面名') actual.add('NOZJJK:' + name.split('·')[0]);
      }
    }
