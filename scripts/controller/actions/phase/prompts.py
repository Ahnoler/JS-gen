...
def filter_expand_try_hint() -> str:
    ...
    return (
        ...
        '「更多/展开」开关的常见识别方式（不绝对，按现场为准）：\n'
        '① 文字按钮：文案为「更多 / 更多条件 / 更多筛选 / 高级筛选 / 展开」；\n'
        '② 纯图标按钮（**不一定有文字**）：外层 class 含 `more-btn`（如 '
        '`<span class="tsscBtn more-btn">` 内嵌 `button.el-button`）；'
        '图标向下 `el-icon-caret-bottom`（或 arrow-down）= **未展开**（点它会展开），'
        '图标向上 `el-icon-caret-top`（或 arrow-up）= **已展开**（不要再点，否则会收起、'
        '反而把字段藏起来）；无文字、可能也无 tooltip；\n'
        '③ 位置线索：通常与「查询/重置」同处一个搜索工具栏。\n'
        '点击方式：优先调用 click_button(\'更多\')（引擎按上述 more-btn/按钮类型识别，'
        '只点未展开的），也可按索引点击该图标按钮。\n'
...
def wizard_nav_task_hint() -> str:
    ...
    return (
        ...
        '   「客户名称」等远程下拉若弹层内是表格行（TsscMultiSelect / .tssc-multi-select），'
        '仍用 select_option(label_text, option_text 或 \"第一个\")，引擎自动走远程表行路径；'
