...
# 旧名兼容别名：U+241F 尾段解析/覆盖逻辑已抽到共享 leaf 模块
# click_locator_tail——同族点击链（switch_tab / click_menu_item / close_dialog /
# click_table_row_button / click_table_row_radio）与 JS_CLICK_ICON_BUTTON 共用；
# 既有 `from .click_action_engine import _apply_click_locator_snapshot` 等导入
# 全部不变（characterize-icon-buttons.py 依赖）。前两个别名仅作兼容再导出。
from .click_locator_tail import (
    LOCATOR_TAIL_SEP as _CLICK_LOCATOR_TAIL_SEP,  # noqa: F401
    click_locator_tail as _click_locator_tail,  # noqa: F401
    apply_click_locator_snapshot as _apply_click_locator_snapshot,
)
