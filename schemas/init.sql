...
CREATE TABLE `trajectory` (
  ...
  `failed_kind`       VARCHAR(48) DEFAULT NULL COMMENT '录制失败机器码（llm_* / phase_failed / quality_failed / zero_step / runner_error / user_marked_failed / batch_failed）；NULL=无失败记录',
  `failed_reason`     VARCHAR(255) DEFAULT NULL COMMENT '录制失败的用户可见类别文案（与前端 toast/列表悬浮一致）；详细原因仅在日志',
  `failed_at`         DATETIME(3) DEFAULT NULL COMMENT '最近一次录制失败时间',
  `created_at`        DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at`        DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
