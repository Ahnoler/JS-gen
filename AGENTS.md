...
- **豁免（2026-09-18 起）：不产生代码改动的任务无需开工声明**——纯真机录制/验收、服务重启、查库核对、写报告类任务（不动 `src/**`/`scripts/**` 源码）直接执行即可，不插开工条目。**但仍须写收工条目**（服务状态、轨迹号、验收证据、遗留），让服务占用/在途状态对他线可见；若过程中临时决定改代码，立即补写开工声明再继续
