...
- 2026-08-28: **菜单切换·第三批（删除拦截 + 执行期导航 + 快照与5.8规则）**。①系统树删除拦截：`deleteNode` 前置校验节点/子树下是否存在交易（`hierarchy-service.assertNodeHasNoTransaction` + `trajectory-dao.countByFunctionIds`），存在则 400 固定文案"当前节点下存在交易数据，不允许删除！"。②执行期导航：回放批执行（`replay-batch-runner.runReplayBatch`）在步骤循环前按交易所属功能的 `menu_xpath` 自动导航（新 `trajectory/menu-navigation.js`：buildMenuNavActions 纯函数构造 click_element_by_index 序列；navigateToFunctionMenu 同菜单跳过（runtime._lastMenuNavKey）/空菜单直接执行/失败告警不阻断；is_replay 恒 true 不入步骤表）。③重复导入快照与删除保留：新表 `system_menu_snapshot`（迁移 20260828110000，整树+页面+交易关联 JSON，导入事务内任何写之前落盘，menu_version 递增）；消失的旧 json_import 菜单在标记 unmatched 后判定——子树无交易则删除（system_page FK CASCADE 随删，含空模块父级清理），有交易保留，返回新增 `snapshotVersion`/`deleted` 统计。
