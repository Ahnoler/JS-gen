...
> 9. **页面 ID 的 UI 来源（截图佐证）**：被测系统页面右上角"？"图标打开"天元相关配置"弹窗，其中**组件编号即页面 ID**（= `managePage.pdCmptEcd`；弹窗内活动名称括号内是活动编号，勿混用）；部分页面无组件编号 → AILZ 生成。因此交易绑定页面 ID（V2）= 录制起点**从页面直读**组件编号，不受菜单-JSON 匹配率（29%）影响——该比率只影响菜单树标注链路。
