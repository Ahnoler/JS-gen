...
> 状态：设计中（待按 08-27 产品流程图修订）
> 日期：2026-08-26
> 范围：JS-gen（AI 录制工具）侧三个模块 + 推送契约；手动录制工具和自动化平台为独立系统，不在本次实现范围内

> **⚠️ 2026-08-27 产品流程图调整**（评审材料见 `docs/需求评审-菜单切换/`），本文档以下内容与流程图存在差异，评审通过后修订：
> 1. **§3.2 JSON 导入**：从"匹配已有系统树"改为**直接初始化系统树**——先按子领域层级建树骨架 + 挂页面 ID，菜单探测结果再校准（中文名匹配，子菜单用父+子路径）。
> 2. **不一致处理**：实际菜单与 JSON 不一致时**以实际系统菜单为准**——实际有而 JSON 无 → 按实际层级新增（非"置空提醒"）；JSON 独有分支处理待评审（建议保留并标记）。
> 3. **菜单录制确认为全自动**探测（原 §8.3 待定项已解决）。
> 4. **执行侧导航两侧都需要**：JS-gen 回放引擎执行交易前按 menu_xpath 导航（本文档 §3.4 设计保留），自动化平台**存储**菜单信息（导入导出文件）并在其执行时做前置导航（同菜单跳过、空菜单直接执行）；JS-gen 另有**录制期**自动导航（仿 runDefaultLogin，交易录制不含导航步骤）。
> 5. **新增交易-菜单关联**：录制时绑定当前菜单（复用 trajectory.function_id），推送 payload 需携带菜单引用 + pageId。
> 6. 导出内容确认为：菜单本身、层级关系、菜单 xpath、页面 ID（原 §8.2 基本解决，xlsx 四列）。
> 7. **2026-08-27 产品文字需求细化版**（三模块逐条，评审材料 §2 为对照基准）：
>    - 自动生成 ID 规则已定 **AILZ+13位时间戳**（原 §8.1 提案 AI_+UUID12 作废）；
>    - 菜单需带**来源**字段（json导入/手动新增/AI识别）；
>    - 重复上传更新细化为 **9 条规则**（快照回退/菜单 ID 迁移/页面 ID 迁移交易/同 ID 改名/新增/同名合并/删除保留/标记未匹配）——本文档 §3.2 的 P1/P2 流程由其取代；
>    - "导出菜单"改为**"推送菜单"按钮直推平台** + 完成回调 + 按钮状态机（文件导出是否保留待确认）；
>    - 交易推送前增加**菜单已同步校验**（menu_version vs pushed_version）；
>    - 系统树删除增加**存在交易禁删**拦截；
>    - 执行期导航两侧都实现（本条修正此前第 4 条"归属平台"的表述）。
> 8. **08-27 评审前与产品沟通确认**：树层级**对齐实际菜单两级**（JSON 顶层子领域→模块、叶子子领域→功能挂顶层祖先、中间层不建节点）；菜单传递**只做"推送菜单"直推按钮**（流程图"导出菜单"环节不实现）；payload 归属关系=**父路径+名称+umlEcd**；平台接收/回调契约尚未沟通（评审需平台到场，剩 Q2/Q3/Q6 见评审材料 §3.2）。
> 9. **页面 ID 的 UI 来源（截图佐证）**：被测系统页面右上角"？"图标打开"天元相关配置"弹窗，其中**组件编号即页面 ID**（= `managePage.pdCmptEcd`）；部分页面无组件编号 → AILZ 生成。因此交易绑定页面 ID（V2）= 录制起点**从页面直读**组件编号，不受菜单-JSON 匹配率（29%）影响——该比率只影响菜单树标注链路。
> 10. **08-28 JSON 调研结论**（脚本 `tmp/analyze_activity_code*.py`）：弹窗"活动名称"整行忽略——名字是任务页名（tskNm），括号码是该活动 guidePage（抽屉）的 pdCmptEcd，名字与码在 JSON 不挂同一节点，括号为 SUT 弹窗渲染拼接；活动自身编码是 UML 格式（umlEcd），ZJJK 码只出现在 pdCmptEcd 字段且 **task 级也带 pdCmptEcd（页面模板实例）——导入解析只收 managePage/guidePages 的 411 个**；需兼容 PDCP 后缀编码；38 个页面被多活动共享（页面→活动 1:N，锚页面不锚活动）；14/111 guidePage 编码为空（AILZ 兜底必要）。
