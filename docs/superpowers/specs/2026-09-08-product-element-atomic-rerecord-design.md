...
**日期**：2026-09-08  
**状态**：已批准（B + 仅参考 #61/#66/#503）；**2026-09-08 16:28 修订方案 A**（废独立 T1）  
...
1. ≥3 条 draft/recorded 轨迹挂载 `functionId=9000000468`，粒度单闭环（T2–T4）；
...
- 自动批量录制；本轮不晋升操作组件
- 修改/删除遗留 #61/#66/#503
- 独立「仅进入/见双根」交易（prepare 已导航；纯核对无落库步）
...
| ID | 名称 | 闭环 | 硬成功判据 |
|---|---|---|---|
| ~~T1~~ | ~~进入产品要素库~~ | **已废（方案 A）** | prepare 导航 + T2 前导见双根；草稿 #693 failed 留档 |
| T2 | 新增要素类型 | 见双根 → 选个性化根 →【新增类型】→ 名称+序号=1 → 保存 | toast「操作成功」+ 树出现 `KB测要素类型-20260908-elem`；`stepCount>0` |
...
```
参考 #61/#66/#503
    → 写 task（T2–T4；T1 文案标废）
    → analyze → create draft（functionId=9000000468）
    → 串行 T2→T3→T4：prepare → start → detach
...
- 按钮双候选：`保存`/`保 存`，`确定`/`确 定`
- 禁止仅核对原子：`done` 在 phase 零动作会被拒（#693）
- prepare 的 `read_page_component_code` 须在 empty-config/timeout 路径也关掉天元弹窗（勿留可见壳给录制 agent）
...
- 2026-09-08 16:28：用户选 **A**——废独立 T1，进入+见双根并入 T2 前导；首录 #694
- 2026-09-08 16:40：用户纠正——天元弹窗导航后应关闭；#694 失败属 page-bind 早退未关窗，非需授权 C
