...
| 评审条目 | spec 覆盖 | 结论 |
|---|---|---|
| 1.【P1】tssc 路由互拒，建议「固定单一动作提示，或两动作互认」 | §一 方案 1 = select 侧落穿（互认）+ 冲突指引文案（单一提示），两建议各取其半且互为犄角 | 一致。**落点修正**：评审线索指 `fill_dispatch.py/select_dispatch.py` 的 tssc 分支——调研证实 `fill_dispatch.py` 不含任何 tssc 判定（只是定位尝试顺序），真实落点 `select_engine.py`；#865 换会话一次成功与根因吻合（store kind 缓存是否在場随会话扫描与否而异，即「路由判定顺序不稳定」的机理） |
| 2.【P2】stepCount 对账，先日志后修 | §二 完全遵评审建议：只加 `[traj-recon]` 四挂点日志，修法留待读数判别 | 一致 |
| 3.【P2】executor 双进程，互斥/自杀退避 | §三 a 补强（锁跨检出）+ c（4001 自杀+401 有限退避）+ b（DB 假活时序） | 一致，且比评审建议多补服务端 upsert 时序缺口 |
| 4.【P3】MySQL deadlock 观察项 | 本批不动 | 一致 |
| 5.【P3】analyze 合并摩擦 | 本批不动（管线体验，非缺陷） | 一致 |
| 6.【另案】引入形态查询条件专用动作 | 不入本 spec——Lead 已裁定（`5956ab7a`）：引擎动作词表冻结，不新增专用动作，如需降摩擦走现有动作内提示词 cue（`scripts/prompts/**`，本批禁入区） | 一致（另案已有结论） |
| 验收提醒：每修配 pin 入 verify-all + 全量基线比对（3 红零新增） | §五 回收验收清单逐字对应 | 一致 |

spec 无需实质修改；本节作为评审闭环记录。
