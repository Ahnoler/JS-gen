...
> 日期：2026-09-10  
> 状态：**待审阅**（批准后进入 [`plans/2026-09-10-recall-p0-three-levers.md`](../plans/2026-09-10-recall-p0-three-levers.md)）  
...
| R6 | 与 `propose.js` 他线冲突 | 本线**不动** `propose.js`；只动 `flow-card-recall.js` + 两个 data 资产 |

## 11. 审阅请确认

1. **D6 的二选一**：(a) 在 v1 上做「构建集/验证集」五五分离（快，今天可做）；(b) 先扩评测集 v2（新独立查询 ≥100 条）再建词表（更干净，但 v2 本身要先立项）。
2. **作用域系数默认值** 1.30 / 0.85 是否接受（还是先取更保守的 1.15 / 0.92）。
3. **DoD 门槛**：T2 的 D 层 ≥0.60、T3 的 B 层 ≥0.35 是否认可（均为下限，非目标）。
4. 是否接受「三杠杆同一 spec、独立 commit 独立回退」的组织方式。
