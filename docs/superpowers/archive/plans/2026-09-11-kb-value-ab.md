...
- **DB 访问（口径已订正）**：**主路径是直连** `47.101.58.49:3306`——由 `config/update-db-whitelist.ps1` 那个**常开窗口每 600s 同步当前出口 IP**（窗口横幅即写明 "You do NOT need open-db-tunnel.cmd anymore"）。**隧道（`ssh -N -L 13306:127.0.0.1:3306`）只是白名单空窗期的兜底**。已知空窗：**VPN 重拨会换出口 IP，白名单最多滞后 10 分钟**——reviewer 实测过一次（01:06 VPN 重拨把隧道重置 `Connection reset`、直连同时超时；01:08:45 同步 OK 后直连 46ms 恢复）。
  → 跑批期间**保持白名单同步窗口开着**；T0 导出与 T3 收数前用**一次真实连接**探活（`SELECT 1`，不要只看 netstat）；收数按 id 查库、**幂等可重跑**，任何时刻断线都不影响已跑完的 run。
  → **注意 `config/.env` 当前指向 `127.0.0.1:13306`（隧道）**（本次会话中被改过，gitignore 无记录）。控制面要连跑数小时，建议指回 `47.101.58.49`，否则隧道一掉控制面就失联。
