...
> 日期：2026-09-09  
> 状态：已实现（2026-09-10）；计划 [`docs/superpowers/plans/2026-09-09-search-then-click.md`](../plans/2026-09-09-search-then-click.md)  
> Lead：本会话 Cursor  
...
`analyzeRequirementToPhases` prompt：阶段文案含「定位/选中树节点或列表行」时，**同一 phase 字符串内**补「先搜索/查询再点击」纪律，且**必须原样保留**用户写明的目标名/取值（禁止抹成笼统「关键字」）；**不增删** phase 条数（遵守既有编号条数硬约束）。查询/搜索阶段录制时注入业务数据（#676）。
