...
> **reviewer 事后更正（2026-09-10，T1b 实测后）**：上表「跨模块误召回 11 条 = 作用域可救」**部分错误**。以单卡语料法实测 gold 卡可见性（`rankFlowCards({cards:[gold], k:1})` 是否返回 `null`）发现：**9 条未转正靶子中 8 条 gold 对打分器完全不可见**（B-006/013/014/022/024/027/028/029 —— 单卡语料仍返回 `null`），乘法系数无法拯救 0 分卡。
> **正确归类**：8 条属**词表鸿沟**、2 条（B-013/B-014）属真排序问题、3 条（B-005/B-010/B-012）可被作用域改善。据此 T1b 判 **FAIL 并整体回退**（见 [`reports/2026-09-10-recall-p0-g-verdict.md`](../reports/2026-09-10-recall-p0-g-verdict.md)）。
> **教训（写入方法学）**：把「结果错」归因为「排序错」之前，必须先做**目标卡可见性检验**；可见性应作为失败分类的前置步骤。
