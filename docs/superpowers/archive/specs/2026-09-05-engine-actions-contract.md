...
| `select_option` | `label_text`、`option_text` | 分流经 `resolve_select_dispatch`（`target_kind` / scan kind / live `.tssc-multi-select` → `tssc` \| `tree` \| `el-select`）；执行经 `SelectEngine`（`select_option_for_replay` / `mode=replay`）。历史 `action_name=tssc_multi_select` 兼容支路同样经引擎（`force_path=tssc`），禁止回放主路径直调 `JS_TSSC_*`。禁止只改 `SelectEngine` 或只改 `replay_form_action`——须改 `select_dispatch` 后双端接线。见 [`2026-09-09-select-record-replay-unify-design.md`](./2026-09-09-select-record-replay-unify-design.md)。 |
| `select_tree_option` | `label_text`、`option_text` | 树选择三段式 |
| `click_radio` | `label_text`、`option_text` | 经 `RadioEngine`（`click_radio_for_replay` / `mode=replay`）：有 xpath → `JS_CLICK_RADIO_BY_XPATH`，否则/失败回退 `JS_CLICK_RADIO`；禁止只改引擎或只改 `replay_form_action`。见 [`2026-09-09-radio-record-replay-unify-design.md`](./2026-09-09-radio-record-replay-unify-design.md)。 |
