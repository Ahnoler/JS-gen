...
## 8. P3-B 补债收官：链B用信支线全链贯通（2026-09-05 15:10）

用户指示「继续补债，A7 影像上传搁置」。用信批复落地即链 B 与链 A 等深：

- **用信单**：YXPC20260905012041（贯通验证企业190416 / 政采贷（流动资金贷款）/ 100,000.00 / 12 月 / 单独发起）→ wf_usecredit_001 全链 001(701994)→002(WN0001)→003(701994)→004(WN0001 同意) → **通过**。
- **用信批复**：**DGYXPF202609050016010** 自动生成·**已生效**（额度 EDBH20260905080002 联动，分项=流动资金贷款额度 CP03988120251147）。
- **链 B 全链至此**：客户→评级生效→授信通过→授信批复生效→**用信通过→用信批复生效**。

本次新实证（详见 tmp/e2e/p3b_report.md，KB 卡 credit_usage +4 规则）：
1. 方案品种产品必须命中授信分项品种（validLmtSubExist），分项额度四字段自动回填=匹配正向信号；
2. 信用担保死路（担保合同无法自建），可行=保证+引入保证人（弹窗行内先填关系+金额再确认）；
3. tssc-multi-select 的 `$emit('input')` 不落 model，须直写 form.model；涉农真键=agrirelLoanInd；
4. DIGT_IDY_CL 列超长 SUT 缺陷（6 位字典码被拒，'0' 绕行）；分区保存失败被 i18n 崩溃吞 toast，用 formComp.validate() 诊断。

## 9. P3-C 链B合同签订推进（2026-09-05 16:40，止于担保合同期限）

- **主合同 9881020044006 自动创建**（批复生效即出现，推翻 A6 手动创建结论）；发起签订进 FS00040626PDCP 签订表单。
- 签订信息全部填齐并保存：双方签署信息/份数 2/受托支付/送达三件套/公证否/**合同账户（放款 803537280+还款主 1125189813）saveCtrAccinf 200**。
- 提交被 `担保合同 988104260032001 期限不能为空` 拦（用信自动创建的担保合同无起止日；主签订页与担保合同管理列表均无编辑入口，需走担保合同自身签订流程——下一步探明）。
- 按开工声明停在不可逆点之前：ctrSt=1 已保存可回退；生效路线（纸质影像 vs 电子签）待用户拍板。KB duigong_contract_sign 卡 +4 规则。报告 tmp/e2e/p3c_report.md。
