...
- 工作范围：`docs/superpowers/specs/2026-09-24-draft-traj-coach-runtime-design.md`、`docs/superpowers/specs/2026-09-20-draft-traj-coach-sop-design.md`（顶部一行交叉链接）、本条目。
- 禁入区：`tools/draft-traj-coach/src/**`、产品 propose/atomize（`src/**`）、他线 WIP。
- 执行方式：主会话直接写入规格与交叉链接；docs-only。
...
- 验收：报告含近 45 天查库基线（成功率约 71%、失败 84 条分类、步间隔与重复步）、五个方向的证据与不做清单、建议顺序。占比已注明是估计；「控件卡住 +10–15 个百分点」已按条数改写为最多约 10 个百分点。
- 遗留：未立项、未改代码。`config/.env` 仍指向隧道地址，本次查库只在临时脚本里直连，未改该文件。

## 2026-09-24 11:20 · Cursor — 开工：录制效率与成功率调研报告

- 授权：用户要求把本会话五路只读调研落成报告，不改产品代码。
- 工作范围：`docs/superpowers/reports/2026-09-24-recording-efficiency-success-research.md`、本条目。
- 禁入区：`scripts/**`、`src/**`、`data/kb/**`、`config/.env`、他线未跟踪的 product-mgmt 草稿。
- 执行方式：主会话根据已完成的调研成文。
...
- 验收：合入后同一句阶段 6 为 `form_fill`、不索要 `query_clicked`；阶段 7 按钮文案为「确认」。
- 遗留：执行机需拉取 `uara_V2.0_dev` 并重启后才会用到这次分类。未做湿测。
...
- 工作范围：`docs/superpowers/plans/2026-09-23-same-family-scan-and-save-negation.md`；设计稿状态改为已审阅；本条目。不改产品代码。
- 禁入区：`scripts/**`、`src/**`、`data/kb/**`。实现等用户选定执行方式后再开工。
...
- **编排教训（自我记过）**：fork 恢复时未核查 G7 仍在途即派 G8——两代理并发操作同一单据 YXPC20260907012045。幸最终状态一致（审批中/节点 2 黄亮待处理/仅一般流动资金贷款 1 条明细），无脏数据；后续派发前必须先清点在途任务。
- **税E贷裁定（G7 提请）**：不恢复。该行=09-21 环境漂移产物（非 09-07 建单内容），两代理殊途同归删除后单据回到原始单一品种状态，与主链一致。
- G9（R6.5 用信审批三节点）继续在途。

## 2026-09-22 11:35 · ZCode Lead — R6 流程提交成功（主链 R1→R6 贯通）+ R6.5 用信审批三节点棒派发
- **G8 完成（R6 收口）**：YXPC20260907012045 【修改】续做→全区块核对→流程提交→**选人弹窗真实 UI 点击选黄亮→「流程提交成功！」→状态=审批中**（节点 2 信贷二次调查=黄亮 WN0001 待处理）。上轮 X0018 拒单确认=信贷侧配置已修复+本轮正规选人弹窗（非绕行）。
- **环境漂移披露（G8）**：用信方案里多出一条 09-21 漂移加入的「税E贷(PD00032012)」空产品行（无金额/无利率/无主担保类型），致担保一致性校验拦（全局流水号 01a0c6f9…）——已 UI 删除该空行（deleteDclScmDtl 200），单据本体完整保留并成功提交。
- **R6.5 已派发（进行中）**：用信审批三节点连贯棒——轨迹 A（WN0001 二次调查）→B（701994 审查）→C（WN0001 终批）→用信批复自动生成核验（R7 合同输入）。
- 前置重盘（G8）：评级 PJ20260907016009=通过 / 授信 DGSX20260907056033=通过 / 批复 DGSXPF20260907020005=生效，前置链路完好；另发现同客户 09-10 新增评级 PJ20260910016010（审批中）非本链单据，未触碰。
...
- 用户定盘：每个版本两套分支，日常只在 dev 上改。V2.0 的稳定 test 沿用现有 `uara_V2.0`，停在 `eab6c041`，不再直接提交。新分支 `uara_V2.0_dev` 从该点切开并已推送；主检出 `D:\dev\JS-gen` 已切到这条。
- 以后新版本成对开 `uara_V<x.y>_dev` 与 `uara_V<x.y>_test`。V2.0 的 test 不另建 `uara_V2.0_test`，稳定线就是 `uara_V2.0`。功能分支从 `uara_V2.0_dev` 切出并合回 dev；要进稳定 test 时再把 dev 合入 `uara_V2.0`。
- 约定已写入 `AGENTS.md` 与 `docs/superpowers/todo-list.md` 页首。其他 worktree（engine / contract / phase-contract）未切换。
- 注：不维护 CHANGELOG。
...
- 完成（6 files / +116 −1655；agent-log 差额系 09-16 及更早条目已归档）：
  - **根因定位**：`JS_VERIFY_FORM_STRUCTURE` 把表格里仅含单选/多选的数据行（业务编号如 `PJ20260910016010`）当作表单字段名，与快照对比后产生 `added_optional`，触发 `healType=form_structure` 自愈；自愈代理用 `click_table_row_radio` 批量点新增行，被 `err-search-first:need-fill-and-query` 拦截，导致回放批次提前结束，后续已勾选步骤未执行，计数显示 1 成功 1 失败。
  - **修复**：在 `scripts/controller/actions/js_snippets/misc.py` 的表单结构校验与 `scripts/controller/actions/js_snippets/scan_form.py` 的表单扫描中，统一跳过「仅含 radio/checkbox 选择控件」的表格行；这些数据行不再被识别为表单字段，避免误触发结构自愈。同步在 `scripts/characterization/cold/characterize-form-structure-container.mjs` 增加 Playwright 冷测用例钉死该行为。
  - **可观测性**：`src/services/trajectory/replay-batch-runner.js` 在批次启动时打印 `traj/steps/stepIds/actions` 摘要；`scripts/controller/actions/_replay.py` 在执行机端打印本次要回放的完整动作清单（含 step id），方便操作人员核对当前回放内容。
- 验收：
  - 定向 characterization 全绿：`characterize-form-structure-container`、`characterize-replay-batch`、`characterize-form-snapshot-trigger`、`characterize-xpath-primary-ops`、`characterize-fill/select/radio/click-replay-engine`。
  - Python 编译：`py_compile` 通过 `misc.py`、`scan_form.py`、`_replay.py`。
