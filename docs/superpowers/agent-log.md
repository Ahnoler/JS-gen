...
- 回链 15:05 开工。完成：`51480bc6`。主模型 `use_vision` 仍为关闭。每步注入 `[门禁]`；门禁已通过的 `done(success=true)` 才单独看视口，看见错误文案才拒绝，同一阶段最多两次；高风险点击在 `multi_act` 前可否决且不写入轨迹，同一阶段最多两次，不确定或调用失败则放行。
