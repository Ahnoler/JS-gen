...
- **KB 召回 P0 三杠杆（2026-09-10 Zcode，✅ 实施完成待 G1/G2/G3）**：Acc@1 0.650→**0.790** / Recall@5 0.757→**0.917** / MRR 0.694→0.843 / nDCG 0.708→0.860 / 拒答 0.633 不变（FP 零新增）/ 噪声 0.790；分层 A 1.00 保持 · B 0.233→0.400 · C 0.867 不动 · D 0.333→0.933。**T2 分词落地**（`tokenizeCodes` 同源分词，9/9 D 层 null 转正，`01572239`）+ **T3 词表落地**（`data/kb/synonyms.json` 16 词条只据 build 集，B 层达标，`2ae8e4e1`）+ **T1a 血缘资产落地**（mapped 72/84，`b0e0e534`）；**T1b 作用域 FAIL 已回退**（11 靶仅 2 转正，7/9 余项 gold raw=0=词面鸿沟非排序问题，系数区间内不可达）；runner 新增 `--synonyms` 模式；`propose.js` 未触碰（A5 放行但作用域回退后无消费者）。**待 reviewer 三 gate**（G1 血缘/词表抽检、G2 划分/冻结/verify-all 未动确认、G3 独立复算+pin 证伪+反作弊）；**裁量点已披露**：T3 词表对 verify 独立增量=0（verify +4 全归因 T2 分词），若判不达标按 A4 词表单项回退；**移交 Lead**：N-002 存量 FP（地板收紧须批）。报告 [`reports/2026-09-10-recall-p0-report.md`](reports/2026-09-10-recall-p0-report.md)
