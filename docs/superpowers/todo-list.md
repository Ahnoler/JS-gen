...
| **d2-503-leg** | P2 | D2 spin-guard 验收 A/C 腿（503 场景）：observation 档命中 `[spin-guard] observed … sut=page_text_503` + soft 档 ~6 步快速失败落 `reason=sut_unavailable_spin_guard`。**B 腿已 PASS**（两轨零误报，2026-09-21）；A/C 腿 BLOCKED=**SUT 侧无法配合制造 503 窗口**（用户裁定，探针蹲守已停）。配方已备 `tmp/contract-wet9-20260919/d2-acceptance-runbook.md`；SUT 自然出现 503（维护窗）时随时可开单（单条 ≤15 分钟）；或引擎线以环境模拟（代理/hosts 伪造 503，需先真实登录再切）自测 |
