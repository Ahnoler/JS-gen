...
- 阶段/映射叶 0811/0812 已建；8 条 traj 已迁。
- **`intermediate_flag`**：非顶层叶子一律 intermediate；扫描为导航 SoT；隔离 E2E `9000000813` 覆盖仅 SUT=0 PASS。
- **待办**：正式 `systemId=1` 全量 scan-menu（误 import 后同名可导航叶缺失，如对公客户管理）；**下周一**安排菜单推送。
