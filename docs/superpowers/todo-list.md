...
| **verify-phase-token** | P2 候选（待评审） | **核验型阶段 token 口径（2026-09-21 合约线移交，非守卫问题）**：#973 P2 纯核验阶段（重搜确认三个已删节点不存在、无保存动作、doneLog 完整）→ 终态 quality_failed；对照 #924 同配方 P2 含删除动作 → recorded。**门**=`scripts/agent/service.py:724-729`（`submit.required && !has_contract_success` → `mark_quality_failed('missing_success_token')`，唯一豁免 `introduce_pick`）；**谓词**=`scripts/controller/actions/phase/intent_gates.py:235-260`（kinds 空→True；否则靠 `_last_save_ok`/introduce/boundary）。**倾向治本**：分析侧对核验型阶段不产出 `submit.required`/kinds（触契约生成）；门侧 `verify` 型豁免为保守兜底。**前置**：先定义「核验型阶段」机械可判定式（success_when 全为只读断言？零写动作？），防给真该失败的空转开后门（v3 假成功防线是核心价值）。**谓词校准对（2026-09-21 合约线回执贡献）**：阳性=#973 阶段2（纯核验：无保存动作、doneLog 完整记录三次重搜「暂无数据」，被 missing_success_token 误杀→quality_failed）；对照=#924 阶段2（核验+删除混合，同配方同页签，token 充分→recorded）——两例同日同实现、差异仅 P2 动作构成，可直接用于判定式校准；需要更大校准集合约线可按 doneLogs+steps 全量盘点历史核验型阶段（点单即出）。**判定式草案 v1 已落（2026-09-21 引擎线，设计稿 specs/2026-09-21-verify-phase-token-caliber-design.md）**：四条机械条件（无保存线索/核验词根+宾语共现/brief_plan 无写动词/mode∈{query,other,navigate} 硬排除）、主收口=apply_phase_contract 唯一写点（须同笔清 boundary.success_when+role→other，has_contract_success 优先读 boundary）、默认节奏=先门侧兜底一个湿测窗口再升主收口；**判定式冻结后才盘点历史集，禁按错分样本回改词表（防反向拟合）；FP 零容忍 FN 可容忍**；mode=verify 死值维持不启用；待用户裁定三项（§5） |
