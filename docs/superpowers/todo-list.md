...
### ⑤ 引擎主链贯通（最高优先 · 2026-09-06 用户定调）

**主链**：客户新增 → 对公评级 → 授信申请 → 审批 → 批复 → 用信 → 合同（对公线）。涉及影像/OCR/文件上传的环节一律绕行（产品裁定 09-05：上传场景搁置，合同止于已保存态待产品排期）。

**能力验收口径（三条，不涉影像/OCR 前提）**：

1. **自主录制**：引擎按 KB 流程卡自主走通环节并经产品管线录制（analyze→create→prepare→record/start→steps 落库）。注意 record/start 假成功模式（产品级待修）——验收认 `stepCount>0` + 业务 stamp，不认「全 phase_done」。
2. **成功回放**：录制产物经 `replay_actions` 回放通过。
3. **LLM 脚本容错**：小页面变化（文案漂移/结构微调）下回放不受影响——关联挂起项 `heal-locate-wet`（Heal-Locate 修复定位路由）。

**现状底子**：P1 纯引擎闭环（YXPC…012040 提交进审批）+ P2 批量自批（累计 8 笔批复）；P3-B 链B等深（授信→批复→用信→用信批复生效）；P3-C 合同已保存态；63 张 pass 卡已经产品管线贯通验证（54 轨迹 recorded）。引擎配方与计划：[`research/2026-09-05-engine-closure-phase2-plan.md`](research/2026-09-05-engine-closure-phase2-plan.md)。

**下一步**：

- [ ] 主链七环节 × 三能力逐环节盘点引擎当前能力差（哪些环节能录能放/哪些缺卡/哪些机制缺口）
- [ ] 按能力差产出 P6 执行计划（引擎 cue / KB 卡补齐 / heal-locate 接入 / 回放验证管线）
- [ ] 每环节验收三证：录制 recorded（stepCount>0）+ 回放通过 + 业务 stamp
