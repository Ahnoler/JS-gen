...
| **recording-redundant-step** | P2（D1 已闭 / D2 待做） | **D（#925 登记，2026-09-20）**：①**D1 已闭（`de18502d`）**——already-matched `select_option` 跨阶段重复落库：新增 `state.has_recorded_field_action`（同字段+同值、只读日志）+ `select_engine.py` 两个 already-matched 分支落库前守卫；**首次已匹配仍落库**（回放保留该步）、后续同字段同值重访不追加；`is_replay` 早返回、人工录制走 `manual_recorder` 直连 `_record_action` 不受影响；新 pin `scripts/characterization/characterize-select-already-matched-dedup.py`（已注册 verify-all）。②**D2 待做（前置未满足）**：SUT 服务端错误（`Service Unavailable`）下目标不可达，阶段仅 10min **无活动** idle watchdog，agent 有活动即不超时 → 同阶段空转到 `max_steps`（#925 实测空转 30min+、落 10 步）。**问题真实性已验证（2026-09-21 系统线，agent-log 收工条目）**：DB 实证轨迹 #925（phase3 永卡 running、10 步、#6=#9/#7=#10 参数级重复、会话存活 31m56s）+ 机理五机制审计全属实（SUT 5xx 零检测、agent 循环仅 done/max_steps/max_failures/stopped/异常五出口、503 页点击硬编码 `ok-clicked` 落库喂 idle watchdog——watchdog 只认 action_log_sync/截图事件、stderr 不喂狗）。**两个实施须吸收的勘误**：①idle watchdog 断言应精确为「有**落库动作**就不触发」（纯未落库读操作空转反而 10min 会触发）；②`phase_error` 现无 `reason` 字段（设计稿新增属加法改动，Node 侧只读 message，兼容）。**前置**：需在线 SUT + 执行机湿测（当前服务停机，离线不可验收），且触及 agent 主循环/prompt；候选方案=「阶段 N 步无新完成任务即 fail」或「SUT 5xx 快速失败」，**须先出设计评审再实施**；设计稿见 `docs/superpowers/specs/2026-09-20-d2-sut-503-spin-guard-design.md` |
