# 总 TODO（2026-08-13 基线 · 2026-09-06 重整 · 2026-09-19 V2.0 时代更新）

> **跨会话共享清单。** Cursor 会话内 TodoWrite 不跨聊天；以本文件为准。  
> 本文件只跟踪**未闭环**项；已交付/已闭环条目不再保留，历史以 git log 为准（根目录 `CHANGELOG.md` 为 2026-09-19 起的对外汇报文档，仅按用户要求更新）。  
...
> **2026-09-18/19 上游换代（用户定盘）**：**`uara_V2.0` 为唯一上游开发分支，V1.2 冻结**（V1.2 全部历史已吸收进 V2.0，含引擎线 B1-B3/B-6 修复与合约线 A 类全批修复，merge `cf8cbe06`）。此后各线：开工声明上游写 `uara_V2.0`，新分支从 V2.0 切。worktree 分工：`D:\dev\JS-gen-engine` = V2.0 全量态（引擎线主场）；`D:\dev\JS-gen-contract` = 合约线湿测证据 worktree（分支 `fix/phase-contract-20260918` 已并入 V2.0，保留回溯用）；4097+LMY 服务仍从 JS-gen-contract 旧态运行，**下次重启窗口改从 engine worktree 启动**（待用户协调）；主检出 `D:\dev\JS-gen` 仍在 V1.2 旧位，切 V2.0 或闲置待用户拍板。
...
- 规格/计划：[`specs/2026-09-07-req-to-draft-traj-design.md`](specs/2026-09-07-req-to-draft-traj-design.md) / [`plans/2026-09-07-req-to-draft-traj.md`](archive/plans/2026-09-07-req-to-draft-traj.md)
...
- **SPA 向导已交付（2026-09-08 SDD Task 1–9）**：规格 [`specs/2026-09-08-req-draft-wizard-ui-design.md`](archive/specs/2026-09-08-req-draft-wizard-ui-design.md) / 计划 [`plans/2026-09-08-req-draft-wizard-ui.md`](archive/plans/2026-09-08-req-draft-wizard-ui.md)；JS-gen `aa4ca8a8`（`hasThroughChains`）；vue-project `37b5219`→`6346e1c`（四步向导 + 录制列表「需求生成草稿」）；Task 9 冒烟 `tmp/req-draft-traj/through-report-wizard-ui.md`（API PASS + 静态无 prepare/record；UI 浏览器湿测未跑）
...
- **流程卡召回 prepare 注入已合入（2026-09-08 SDD）**：`1ad954fe`→`5cca1e8d`；spec [`specs/2026-09-08-atom-record-flow-card-recall-design.md`](archive/specs/2026-09-08-atom-record-flow-card-recall-design.md) / plan [`plans/2026-09-08-atom-record-flow-card-recall.md`](archive/plans/2026-09-08-atom-record-flow-card-recall.md)；⑧ 测试缺 `【流程卡模板】`→查 migrate + traj `kbFlowRef` + 卡文件
- **KB 链路加固已落地（2026-09-08 加固线，13 任务 ALL GREEN，`fb9f3a5b..9caadef9`）**：atomKey 稳定化+回填、幂等全状态+`req_atom_seq` 唯一索引、propose 缓存 cacheVersion/sourceHash 失效、4xx 错误语义、出处锚点 `req_source_hash`/`req_chunk_id`+commit 回查、召回 idf 重写（800 字 654ms→2ms）+跨语言金样例契约、validate 端点、paasUserId、functionIdCandidates、reference_step 守卫+truncated、观测 JSONL、source 上传、promotedAt；spec [`specs/2026-09-08-kb-remediation-design.md`](archive/specs/2026-09-08-kb-remediation-design.md)
- **reviewer 验收 + 修复收尾（2026-09-09 DSH reviewer）**：结论 **PASS**（初次 DONE_WITH_CONCERNS → 修复后复验全绿，门禁 ALL GREEN / 五条基线 OK 53·15·11·11·24+5div / lint 68→65 无 error）；修复 F-A 新增 lint warning、F-B 观测落盘隔离（`KB_STAGING_DIR`）+清污（保留 4 条真实 py 召回）、F-C `matchFlowForAtom` 返回 `score`、F-D 金样例 note 事实更正、R-3 py 绊线改「不得有已收敛却仍登记的分歧」、R-4 出处列入 api-docs；报告 [`reports/2026-09-09-kb-remediation-reviewer-verdict.md`](archive/reports/2026-09-09-kb-remediation-reviewer-verdict.md)
- **§6.4 前端派单已落地（2026-09-09 SDD）**：Vue `a1ac7d1`→`587f30c`（per-atom 功能下拉 + `functionIdOverrides` 按行提交）；spec [`2026-09-09-req-draft-wizard-function-candidates-design.md`](archive/specs/2026-09-09-req-draft-wizard-function-candidates-design.md)；待 4097 重启 + product-mgmt 湿测（多行不同功能 overrides）
...
- **wet9 产品管理湿测线（2026-09-19/20 合约线，主体已闭环）**：#897–#924 共 12 单真机录制（fid 9000000740，acct 2），证据 `tmp/contract-wet9-20260919/`、`tmp/contract-wet9-20260920-pdiag/`。
  ...
  - **未闭环**：①~~残留族业务清理~~ **已清零**（2026-09-20 Playwright 清理单 `tmp/contract-wet9-20260920-cleanup/`：8 节点全删净——PD00044268/69/70/74/76/78 + wet9阶段V/U-20260920；**机制=两类拦截皆为纯前端预校验**〔「删除仅限未启用」pdSt 静默吞掉无请求、「已存在产品引用」delStg 查关联即拒〕，页面登录态复刻请求/先清 `prodPdStgRel` 悬挂关联后删阶段，后端均 200）；②~~「选择阶段」弹窗交互配方待落 KB 卡~~ **已落并并入 V2.0**（2026-09-20 `product_library.json`：节点 `prod_stage_assoc_dlg` + 规则「设置管控要素前置=已关联阶段／关联不随产品删除级联」；双侧金样例 26 passed + py ok + recall-eval 逐字段零差异；合并 `08afc16c`）；③~~B 类五项测试报告~~ **已交付**（2026-09-20 按归属拆两份：`docs/reports/2026-09-20-b-class-handover-engine-line.md` 3 项〔TsscMultiSelect 路由互拒待修 / stepNumber 空号已闭环 / analyze 粒度策略态〕+ `docs/reports/2026-09-20-b-class-handover-system-line.md` 2 项〔executor 双实例互踢待守护 / MySQL deadlock 根因待裁〕；含口径校正与分发建议）。报告：`through-report-b3v/b3t/sixth.md`、`pdiag-report.md`、`cleanup-99-final-verification.json`。
...
### ⑤ KB-I5 引擎自主闭环（P1/P2 已达成 · 2026-09-05，转下阶段）——本线已于 2026-09-06 并入上方「⑤ 引擎主链贯通」，历史配方与 P3 明细见 [`research/2026-09-05-engine-closure-phase2-plan.md`](archive/research/2026-09-05-engine-closure-phase2-plan.md)
...
| **L1c-wet** | P1 已闭 | `L1C_LLM=true` 湿测 PASS（2026-09-11）：classify 低置信 `source=llm` + L1d 二次命中 + 高置信 rule；BiB resolve traj 678 inventory 通路 OK（本页无低置信区）。报告 [`reports/2026-09-11-l1c-wet.md`](archive/reports/2026-09-11-l1c-wet.md) |
...
| **d2-503-leg** | P2 · 值守 | D2 spin-guard A/C 腿终局口径（引擎线 2026-09-21 裁定，合约线采纳）：**主路径=自然窗口**——observation 档本机运行态**常开**（控制面 pid 9908 + 执行机 32220 已注入），自然 503 出现即按 `tmp/contract-wet9-20260919/d2-acceptance-runbook.md` 开单（≤15 分钟，grep 稳定签名=`[spin-guard] observed` + `sut=page_text_503`，勿逐字 grep `2/2`——live 特例，生产 6 步窗）。**加速路径（本地 503 代理）暂不点单**（已交付并入 V2.0 eab5e83c、零重启零外部依赖，随时可开；CDP 路由注入已被引擎线否决，证据 `tmp/d2-accept-sim/README.md`）；组件级半程已由引擎线补齐：live 实测脚本（c87b9be7，真实 chromium+真实探测 JS，23 断言）闭合离线 pin 结构性盲区，腿 A stderr 形态可逐字复现；未验面收敛为全管线 E2E，由主路径闭合。**常开红利**：后续每单湿测 stderr grep 纳入常规验收，顺带积累 spin-guard 真机证据 |
| **recording-redundant-step** | P2（D1 已闭 / D2 已并入待湿测） | **D（#925 登记，2026-09-20）**：①**D1 已闭（`de18502d`）**——already-matched `select_option` 跨阶段重复落库：新增 `state.has_recorded_field_action`（同字段+同值、只读日志）+ `select_engine.py` 两个 already-matched 分支落库前守卫；**首次已匹配仍落库**（回放保留该步）、后续同字段同值重访不追加；`is_replay` 早返回、人工录制走 `manual_recorder` 直连 `_record_action` 不受影响；新 pin `scripts/characterization/characterize-select-already-matched-dedup.py`（已注册 verify-all）。②**D2 已并入 V2.0（2026-09-21 用户批，`0cbcbd35`；交付分支 `engine/d2-spin-guard-20260921`=fec9b5e3，SDD 全流程+终审 F1 幂等双键修正见其 agent-log 收工条目）**——SUT 服务端错误（`Service Unavailable`）下同阶段空转到 `max_steps`。**问题真实性已验证（2026-09-21 系统线，agent-log 收工条目）**：DB 实证轨迹 #925（phase3 永卡 running、10 步、#6=#9/#7=#10 参数级重复、会话存活 31m56s）+ 机理五机制审计全属实（SUT 5xx 零检测、agent 循环仅 done/max_steps/max_failures/stopped/异常五出口、503 页点击硬编码 `ok-clicked` 落库喂 idle watchdog——watchdog 只认 action_log_sync/截图事件、stderr 不喂狗）。**两个勘误已吸收**：①idle watchdog 断言精确为「有**落库动作**就不触发」（纯未落库读操作空转反而 10min 会触发，观测模式以此为边界）；②`phase_error` 加 `reason` 字段=加法改动，Node 侧只读 message，兼容无破坏。**实施**：`on_step_end` 双条件守卫（A=SUT 不可达页面信号 A1 文本/A3 URL/A4 DOM 缺失，**A2 网络 5xx 缓发**——`network_capture` 只发内存事件、store 无键读取路径不存在；B=最近 N 步无进展，三信号=done 数增长/pathname 变化/新容器首开〔重开已见容器不算〕），`SUT_SPIN_GUARD_MODE` 四档 off/observation/soft/hard **默认 off**（合并零行为影响），soft/hard 触发=直发 `phase_error(reason='sut_unavailable_spin_guard')`+停 agent+续跑循环 break；pin `characterize-sut-spin-guard`（91 断言，含终审 F1 后 10a/10b/10c）已注册 verify-all；三裁定全文见设计稿 §12「实施记录」。**验收（2026-09-21 合约线）**：**B 腿 PASS**（#972/#973 两条正常长轨迹 observation 档零误报零误杀、步号连续；证据 `tmp/contract-wet9-20260919/d2-acceptance/`）；**组件级半程 PASS**（`characterize-sut-spin-guard-live.py`：真实 chromium+真实探测 JS 23 断言，并入 V2.0 `c87b9be7`——钉掉 pin 假页面盲区〔探测 JS 此前从未在真实 DOM 执行，笔误会被守卫 try/except 静默吞掉〕）；**A/C 腿 BLOCKED**（SUT 503 窗口不可得，用户裁定 SUT 侧不配合）。**A/C 裁定（引擎线）**：主路径=自然窗口（observation 档运行态常开，Runbook 随时开单）；加速路径=**已交付并并入 V2.0（2026-09-21 用户批「这个 flag 可以加」，`eab5e83c`；分支 `engine/d2-proxy-accel-20260921`=14bcbb9e）**——`factory.py` flag 文件门控 `--proxy-server`（`tmp/chrome-proxy.flag` 按会话即时读取，默认态零行为影响、无需重启）+ 代理本体 `tmp/d2-accept-sim/d2-503-proxy.py`（转发/503 双模式、host 过滤、随时 ARM/disarm）+ `PROXY-RUNBOOK.md`（合约线执行版）；验收=离线 pin 23 断言（注册 verify-all）+ 真件端到端 6 断言（假域名 `sut-selfcheck.invalid`+`--map` 绕 **Chromium loopback 隐式代理绕过**——localhost 直连会让 e2e 假阳性，负控「代理死→导航必败」实证 flag 位生效；on-demand 不注册）+ selfcheck 4/4 + 合并态 verify-all 196 ok/3 已知红零新增；**CDP 路由注入已否决**（跨客户端拦截致请求暂停无人应答、上生产浏览器会卡死验收腿；证据 `tmp/d2-accept-sim/README.md`）；残余风险=全管线 E2E 未验（非逻辑未验）。**A/C 终局口径（2026-09-21 合约线回执 5afce187，引擎线存档）**：主路径=自然窗口值守——observation 档常开（pid 9908/执行机 32220），自然 503 出现即按 runbook 开单（≤15 分钟）；加速件已备**未点单**（合约线裁定；注：交付版已零重启零外部依赖，其不点单理由「需重启窗口」基于旧 env 方案——再点单成本≈零，随时可开）；合约线对等承诺=每单湿测 executor stderr grep `[spin-guard]`/`phase_error` 入常规验收清单，有命中即回执。**grep 签名精确口径（引擎线更正，防漏报）**：稳定部分=`[spin-guard] observed` + `sut=page_text_503`（#925 形态 A1）；**`progress_window=2/2` 是 live 校验 2 步窗特例，勿逐字 grep**——生产默认 6 步窗，真机命中为 `progress_window=6/6`；A3/A4 命中形态分别 `sut=url_error_page`/`sut=dom_missing`。残余风险=全管线 E2E 未验（由主路径闭合，非逻辑未验，合约线按此口径记账）。**余下**：自然窗口命中 → observation 证据归档 → 逐级升档（soft 需重启窗口用户批，hard 须 Lead 批） |
| **verify-phase-token** | P2（第一步门侧兜底已并入，等湿测窗口；主收口另单） | **核验型阶段 token 口径（2026-09-21 合约线移交，非守卫问题）**：#973 P2 纯核验阶段（重搜确认三个已删节点不存在、无保存动作、doneLog 完整）→ 终态 quality_failed；对照 #924 同配方 P2 含删除动作 → recorded。**门**=`scripts/agent/service.py:724-729`（`submit.required && !has_contract_success` → `mark_quality_failed('missing_success_token')`，唯一豁免 `introduce_pick`）；**谓词**=`scripts/controller/actions/phase/intent_gates.py:235-260`（kinds 空→True；否则靠 `_last_save_ok`/introduce/boundary）。**倾向治本**：分析侧对核验型阶段不产出 `submit.required`/kinds（触契约生成）；门侧 `verify` 型豁免为保守兜底。**前置**：先定义「核验型阶段」机械可判定式（success_when 全为只读断言？零写动作？），防给真该失败的空转开后门（v3 假成功防线是核心价值）。**谓词校准对（2026-09-21 合约线回执贡献）**：阳性=#973 阶段2（纯核验：无保存动作、doneLog 完整记录三次重搜「暂无数据」，被 missing_success_token 误杀→quality_failed）；对照=#924 阶段2（核验+删除混合，同配方同页签，token 充分→recorded）——两例同日同实现、差异仅 P2 动作构成，可直接用于判定式校准；需要更大校准集合约线可按 doneLogs+steps 全量盘点历史核验型阶段（点单即出）。**判定式草案 v1 已落（2026-09-21 引擎线，设计稿 specs/2026-09-21-verify-phase-token-caliber-design.md）**：四条机械条件（无保存线索/核验词根+宾语共现/brief_plan 无写动词/mode∈{query,other,navigate} 硬排除）、主收口=apply_phase_contract 唯一写点（须同笔清 boundary.success_when+role→other，has_contract_success 优先读 boundary）、默认节奏=先门侧兜底一个湿测窗口再升主收口；**判定式冻结后才盘点历史集，禁按错分样本回改词表（防反向拟合）；FP 零容忍 FN 可容忍**；mode=verify 死值维持不启用；待用户裁定三项（§5）（三裁定已按推荐执行：判定式 v1 冻结/先门侧兜底/verify 死值维持——2026-09-21 用户「继续」）。**第一步已并入 V2.0（2026-09-21 引擎线，2e4c24fe→合并 c55cc849）**：verification_gate.py 纯函数（四条件+±16 字共现窗+黑名单扫前掩蔽宾语状态词=「已删除」不杀 #973、裸「删除」仍杀 #924）+ service.py 收尾门 introduce_pick 后追加豁免；pin 40 断言入 core 域；合并后验收 202 项判定 ALL GREEN（layer-tree/step-highlight 两红经 BASE stash 往返归因=共享录制库数据漂移非本线）。**第二步主收口（apply_phase_contract+boundary 同笔清 success_when/role→other）待湿测窗口零 FP 后点单**；条件②「不存在」根词自共现词表面与 verify-all 并行 runner 缺陷（FAILED=1 后台子 shell 不阻退出）已登记待办 |
