...
| **recording-redundant-step** | P2（D1 已闭 / D2 代码已落地待湿测） | **D（#925 登记，2026-09-20）**：①**D1 已闭（`de18502d`）**——already-matched `select_option` 跨阶段重复落库：新增 `state.has_recorded_field_action`（同字段+同值、只读日志）+ `select_engine.py` 两个 already-matched 分支落库前守卫；**首次已匹配仍落库**（回放保留该步）、后续同字段同值重访不追加；`is_replay` 早返回、人工录制走 `manual_recorder` 直连 `_record_action` 不受影响；新 pin `scripts/characterization/characterize-select-already-matched-dedup.py`（已注册 verify-all）。②**D2 代码已落地（2026-09-21 引擎线，分支 `engine/d2-spin-guard-20260921`，默认 off 未合并待批）**——SUT 服务端错误（`Service Unavailable`）下同阶段空转到 `max_steps`。**问题真实性已验证（2026-09-21 系统线，agent-log 收工条目）**：DB 实证轨迹 #925（phase3 永卡 running、10 步、#6=#9/#7=#10 参数级重复、会话存活 31m56s）+ 机理五机制审计全属实（SUT 5xx 零检测、agent 循环仅 done/max_steps/max_failures/stopped/异常五出口、503 页点击硬编码 `ok-clicked` 落库喂 idle watchdog——watchdog 只认 action_log_sync/截图事件、stderr 不喂狗）。**两个勘误已吸收**：①idle watchdog 断言精确为「有**落库动作**就不触发」（纯未落库读操作空转反而 10min 会触发，观测模式以此为边界）；②`phase_error` 加 `reason` 字段=加法改动，Node 侧只读 message，兼容无破坏。**实施**：`on_step_end` 双条件守卫（A=SUT 不可达页面信号 A1 文本/A3 URL/A4 DOM 缺失，**A2 网络 5xx 缓发**——`network_capture` 只发内存事件、store 无键读取路径不存在；B=最近 N 步无进展，三信号=done 数增长/pathname 变化/新容器首开〔重开已见容器不算〕），`SUT_SPIN_GUARD_MODE` 四档 off/observation/soft/hard **默认 off**，soft/hard 触发=直发 `phase_error(reason='sut_unavailable_spin_guard')`+停 agent+续跑循环 break；pin `characterize-sut-spin-guard`（73 断言 RED→GREEN）已注册 verify-all；三裁定全文见设计稿末尾「实施记录」节。**余下（湿测前置：在线 SUT+执行机，当前停机）**：observation 档 #925 复现场景必须命中 + 正常长阶段（多轮填写/树搜索/分页）不得误杀 → 按湿测数据逐级升档；幂等=phase+runId 双键作用域（终审 F1 修正，重录路径同相位重入实证存在，pin 10a/10b 钉死） |
