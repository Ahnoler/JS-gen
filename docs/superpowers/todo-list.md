...
- **KB 覆盖回溯（2026-09-11 Zcode，方向 A，✅ 线交付完成待 reviewer 四项复核）**：416 条真实录制 × 84 卡量出六项基线并固化门禁——**M1 可连接率 0.857**（72/84，12 张卡零码结构性失联）/ **M2 覆盖率 0.206**（69/335）/ **M3 利用率 0.321**（27/84，57 死卡）/ M4 缺口 266 轨迹（Top20 中 18 条为 AILZ 组件码=映射体系外）/ **M5 节点一致性 0.131·顺序 0.012·卡外率 0.835**（n=64——卡节点序列与实跑系统性脱节，佐证轨迹反哺卡节点方向）/ **M6 新鲜度 0.250**（33/132 码存活，38 张陈旧卡）。映射链 ZJJK 58/FS 0/ROUTE 58（FS 链全库仅 1 URL 有 fcnScnEcd）；歧义率 0.101。**结论：KB 现状=少数高频交易的深知识而非全行广覆盖；投资方向=做准 27 张活卡+按真实频次补缺口（最大单点=补 rating 卡对公客户评级申请页 ZJJK 码×6 轨迹），不先扩卡数**。资产：快照 `scripts/kb/coverage-snapshot.mjs`（只 SELECT+白名单脱敏，SNAPSHOT_DB_HOST/PORT env 覆盖不碰共享 .env）+ 冻结 fixture `kb-coverage.v1.json`（data sha `375329fde791d4a9`）+ 引擎 `scripts/kb/kb-coverage.mjs`（确定性连跑逐位一致）+ 门禁 `characterize-kb-coverage.mjs`（脱敏硬断言+floor lockstep，证伪双钩子 `KB_COVERAGE_FIXTURE`/`KB_COVERAGE_BASELINE` 各实证必红）。报告 [`reports/2026-09-11-kb-coverage-retro.md`](reports/2026-09-11-kb-coverage-retro.md)；提交链 `ca4763b6`→`8612b6c4`→`c52b929c`→`dfa3a1c2`→`d5de67dc`→`5b05c5d8`。**遗留（Lead/reviewer）**：复核四项（抽检回库/独立复算/证伪复跑/口径确认）；补码建卡处置=卡治理另立项
