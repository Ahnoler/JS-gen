...
| **PR-LAYER** | **第一刀已实现** | 元素分层树（分区之后） | 每控件 `layers[]` 已落 snap/resolve preview/扫描/`element_json`；可选 `pageLabel` 只加根 page；todo role 已对齐。**整页大树已落地（assembleRegionTree + 扫描/阶段树）** | 依赖 PR-PART；Vue 画树另刀 |
...
| **PR-PUSH** | **已完成** · V2.0：每步 regionId/parentRegionId + 每交易 phases[]（截图引用+元数据）— [spec](specs/2026-08-14-batch-push-v2-region-evidence-design.md) | 推送到自动化 | 拒草稿；仅 recorded/completed | export-push-gate |
...
- [ ] 梳理 Replay → Action → Locator → Heal 调用链
- [ ] 确认 Heal 触发条件
- [ ] 分析当前 heal prompt 输入信息
- [ ] 梳理当前 Locator 能力
- [ ] 收集已有失败案例
- [ ] 输出当前 Heal-Locate 架构图

产出：当前实现分析文档。

### Phase 1：Heal-Locate Design

- [ ] 定义 Missing Element 分类体系
- [ ] 定义 Heal Decision Tree
- [ ] 定义定位优先级
- [ ] 定义 Repair / Skip / Retry 策略
