...
- **DB 只经 SSH 隧道**（`ssh -N -L 13306:127.0.0.1:3306 root@47.101.58.49`；直连被白名单挡）。**隧道会中途掉线**（reviewer 实测：2026-09-11 01:06 一条隧道跑约 35 分钟后 `client_loop: send disconnect: Connection reset`，自然退出 exit 1；VPN 本身未断——LLM 网关始终可达）。故：**T0 导出与 T3 收数前先确认 `127.0.0.1:13306` LISTENING，掉了就重开再继续**；收数按 id 查库、幂等可重跑，任何时刻断线都不影响已跑完的 run。
