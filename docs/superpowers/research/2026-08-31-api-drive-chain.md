...
## 10. KB 召回实测（2026-09-01，VPN 解封后）

### 10.1 KB 召回湿测（交易 211/212，脚本 tmp/kb_live_check.py）
- `export_dicts` → 入库 `data/kb/dicts_normalized.json`（**kb_dict 随即命中** `cstSt` 6 项含信贷正式客户）；`kb_flow(对公授信申请)` 命中 nextBefore 闸门；`kb_field(上市公司标志)` 命中依赖组——**四项召回全绿**（KB-1/2/3 批次验收通过）。
- 注意：SPA 启动后字典缓存异步写入 localStorage，prepare 完成后**立即** export 可能拿到空/未落缓存（首轮 1/2 步 False 的原因）——export 前等首页渲染完成（本脚本 sleep 2 已足够，第二轮全过）。

### 10.2 KB-4 有/无召回 A/B 对照（脚本 tmp/kb_ab_drill.py，用信申请流程）
| 轮 | 方式 | 无效操作(err-*) | 动作数 | 耗时 | 目标达成 |
|---|---|---|---|---|---|
| A 基线 | 无召回，假定查询区可填 | **3**（err-field-disabled ×3，查询区三框 disabled） | 9 | 68.2s | 引入弹窗打开 |
...
### cu2 补采入库（2026-09-01）
- `data/kb/flows/credit_application.json` 已替换为**向导全链卡**：6 向导步全字段、授信方案 7 区清单（含 获取理论值=押品×抵押率/占用余额/额度分项 等依赖）、双重硬前置（生效评级+无在途申请，均 BizException 原文）、状态机扩展（打回/流程取回限制）、工作流元数据（wf_credit_001、信贷调查→二次调查、流程选人机构树）。
- 撤销清理定案：审批中申请 initiator 不可撤销（产品状态机），DGSX20260901056026 留在 黄亮(WN0001) 待办由人处理。
